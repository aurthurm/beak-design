import type { NodeType } from "@ha/pencil-editor";
...
// TODO(sedivy): I took this from the existing image code. I don't
// think this is the corect solution to handle both vscode and web.
export function getRequestOrigin(): string {
  if (typeof window !== "undefined" && window.webappapi) {
    return window.webappapi.getBasePath();
  }

  if (typeof window !== "undefined" && window.vscodeapi) {
    // Check if we're in dev mode by looking at the script tags
    const scripts = document.getElementsByTagName("script");
    const hasDevScript = Array.from(scripts).some((script) =>
      script.src.includes("localhost:3000"),
    );

    if (hasDevScript) {
      return "http://localhost:3000/";
    }
    // For VSCode webview in production env, use the injected base URI.
    if ((window as any).VSCODE_WEBVIEW_BASE_URI) {
      return (window as any).VSCODE_WEBVIEW_BASE_URI;
    }
    // Fallback to relative paths for local files
    return "./";
  }
...
export function getNodeTypeName(type: NodeType) {
  switch (type) {
    case "group":
      return "Group";
    case "frame":
