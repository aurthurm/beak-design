...
export function ConnectionSettings() {
  ...
  const [ollama, setOllama] = useState<DetectedAgent | null>(null)
  ...
  const loadAgents = async () => {
    if (!isTauri()) {
      setError('Connection detection requires Tauri environment (desktop app)')
      return
    }
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="mt-2 space-y-1 text-sm text-muted-foreground">
              ...
              {agent.metadata?.models && agent.metadata.models.length > 0 && (
                <div>
                  <span className="font-medium">Models:</span>{' '}
                  {agent.metadata.models.slice(0, 3).join(', ')}
                  {agent.metadata.models.length > 3 &&
                    ` (+${agent.metadata.models.length - 3} more)`}
                </div>
              )}
            </div>
          </div>
  ...
  if (!isTauri()) {
    return (
      <Alert>
        ...
        <p>Connection detection is only available in the desktop app (Tauri).</p>
      </Alert>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-medium">Agent Connections</h3>
          <p className="text-sm text-muted-foreground">
            Detected local agents and their connection status
          </p>
        </div>
      ...
      {/* No agents detected */}
      {!loading && !ollama && cliTools.length === 0 && mcpServers.length === 0 && (
        <Card className="p-6 text-center text-muted-foreground">
          <p>No local agents detected.</p>
          <p className="text-sm mt-2">
            Install Ollama, CLI tools, or configure MCP servers to get started.
          </p>
        </Card>
      )}
    </div>
